...
import CreateDesignationModal from './create/page'
import EditDesignationModal from './edit/page'
import ViewDesignationModal from './view/page'
...
const DesignationList = () => {
  ...
  const fetchDesignations = async () => {
    try {
      ...
      const res = await axios.get('https://api.doubtsclear.com/api/admin/designation', {
  ...
  const handleDelete = async (designationId) => {
    if (confirm('Are you sure you want to delete this designation?')) {
      try {
        const token = localStorage.getItem("token")
        await axios.delete(`https://api.doubtsclear.com/api/admin/designation/${designationId}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
        fetchDesignations()
      } catch (err) {
    ...
    <div className="flex min-h-screen bg-black">
      ...
      <main className="flex-1 p-6 bg-white">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-800">Designation Management</h1>
          <button
            onClick={() => setShowCreateModal(true)}
            className="flex items-center px-4 py-2 bg-[#E53935] text-white hover:bg-black"
          >
            <FiPlus className="mr-2" /> Create Designation
          </button>
        </div>
                ...
                {currentItems.length > 0 ? (
                  currentItems.map(designation => (
                    <tr key={designation._id} className="border-b border-gray-200 hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{designation._id}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-md font-medium text-gray-900">{designation.name}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {new Date(designation.createdAt).toLocaleDateString()}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          <button onClick={() => openViewModal(designation)} className="text-blue-600 hover:text-blue-900">
                            <FiEye />
                          </button>
                          <button onClick={() => openEditModal(designation)} className="text-yellow-600 hover:text-yellow-900">
                            <FiEdit2 />
                          </button>
                          <button onClick={() => handleDelete(designation._id)} className="text-red-600 hover:text-red-900">
                            <FiTrash2 />
                          </button>
                        </div>
                      </td>
        ...
        {showCreateModal && (
          <CreateDesignationModal 
            ...
            onDesignationCreated={fetchDesignations}
          />
        )}
