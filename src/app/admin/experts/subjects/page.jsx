...
import CreateSubjectModal from './create/page'
import EditSubjectModal from './edit/page'
import ViewSubjectModal from './view/page'
...
const SubjectList = () => {
  ...
  const fetchSubjects = async () => {
    try {
      ...
      const res = await axios.get('https://api.doubtsclear.com/api/admin/subject', {
  ...
  const handleDelete = async (subjectId) => {
    if (confirm('Are you sure you want to delete this subject?')) {
      try {
        const token = localStorage.getItem("token")
        await axios.delete(`https://api.doubtsclear.com/api/admin/subject/${subjectId}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
        fetchSubjects()
      } catch (err) {
                ...
                {currentItems.length > 0 ? (
                  currentItems.map(subject => (
                    <tr key={subject._id} className="border-b border-gray-200 hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{subject._id}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-md font-medium text-gray-900">{subject.name}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {new Date(subject.createdAt).toLocaleDateString()}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          <button onClick={() => openViewModal(subject)} className="text-blue-600 hover:text-blue-900">
                            <FiEye />
                          </button>
                          <button onClick={() => openEditModal(subject)} className="text-yellow-600 hover:text-yellow-900">
                            <FiEdit2 />
                          </button>
