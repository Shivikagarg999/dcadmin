...
const EditQualificationModal = ({ qualification, onClose, onQualificationUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://api.doubtsclear.com/api/admin/qualification/${qualification._id}`,
        { name },
        {
          headers: { 
      ...
      toast.error(err.response?.data?.message || err.message || 'Failed to update qualification')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-md">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-xl font-bold">Edit Qualification</h2>
          <button 
            onClick={onClose} 
